...
|-----------|----------|---------|-------------|
| `git.repoUrl` | No | `https://github.com/openchoreo/community-modules.git` | Git repository URL (HTTPS) that contains the Terraform files. Override to use your own repo. |
| `git.branch` | No | `main` | Branch or tag to check out |
| `git.tfPath` | No | `workflow-azure-sql-database/terraform` | Relative path inside the cloned repo to the directory containing the Terraform files |
...
spec:
  workflow:
    ...
    parameters:
      git:
        repoUrl: "https://github.com/openchoreo/community-modules.git"
        branch: "main"
        tfPath: "workflow-azure-sql-database/terraform"
      azure:
        location: "eastus"
