...
func TestParseSNSEnvelopeNotification(t *testing.T) {
	alarmName := BuildAlarmName("payments", "high-cpu")
	body := []byte(`{
		"Type": "Notification",
		"MessageId": "msg-1",
		"TopicArn": "arn:aws:sns:eu-north-1:123456789012:alerts",
		"Timestamp": "2026-04-23T10:00:00Z",
		"Message": "{\"AlarmName\":\"` + alarmName + `\",\"NewStateValue\":\"ALARM\",\"NewStateReason\":\"Threshold Crossed: 1 datapoint [5.0 (23/04/26 10:00:00)] was greater than the threshold (3.0).\",\"StateChangeTime\":\"2026-04-23T10:00:05Z\"}"
	}`)
	...
	if res.Event.AlarmName != alarmName {
		t.Fatalf("expected alarm name preserved, got %q", res.Event.AlarmName)
	}
...
func TestParseSNSEnvelopeSubscriptionConfirmation(t *testing.T) {
	...
	if res.SubscribeURL == "" || res.Token == "" {
		t.Fatalf("expected subscription fields preserved: %#v", res)
	}
}

func TestParseSNSEnvelopeRejectsUnknownType(t *testing.T) {
	if _, err := ParseSNSEnvelope([]byte(`{"Type":"Mystery","Message":"x"}`)); err == nil {
...
func TestParseEventBridgeEvent(t *testing.T) {
	...
	if evt.AlarmName != alarmName {
		t.Fatalf("expected alarm name preserved: %#v", evt)
	}
}
...
func TestParseEventBridgeEventRejectsUnexpectedSource(t *testing.T) {
	...
	if _, err := ParseEventBridgeEvent(body); err == nil {
		t.Fatal("expected source filter to reject")
	}
}
...
func TestParseEventBridgeEventFallsBackToReasonText(t *testing.T) {
	body := []byte(`{
		"source": "aws.cloudwatch",
		"detail": {
			"alarmName": "x",
			"state": {
				"value": "ALARM",
				"reason": "Threshold Crossed: 1 datapoint [9.5 (23/04/26 10:00:00)] was greater than the threshold (3.0).",
				"timestamp": "2026-04-23T10:00:05Z"
			}
		}
	}`)
	...
	if evt.AlertValue != 9.5 {
		t.Fatalf("expected reason fallback to extract 9.5, got %v", evt.AlertValue)
	}
}

func TestParseLambdaForwarderEvent(t *testing.T) {
	body := []byte(`{
		"alarmName": "oc-metrics-alert-123",
		"ruleName": "high-cpu",
	...
	if evt.RuleName != "high-cpu" || evt.RuleNamespace != "payments" {
...
func TestApplyTagsToEvent(t *testing.T) {
	evt := &ParsedAlertEvent{AlarmName: "oc-metrics-alert-123"}
	ApplyTagsToEvent(evt, map[string]string{
		TagRuleName:      "high-cpu",
...
func TestApplyTagsToEventIgnoresEmptyValues(t *testing.T) {
	...
	if evt.RuleName != "existing" {
		t.Fatalf("did not expect empty tag to overwrite, got %q", evt.RuleName)
	}
}
...
func TestExtractDatapointFromReasonDataHandlesInvalidJSON(t *testing.T) {
	if got := extractDatapointFromReasonData("{"); got != 0 {
		t.Fatalf("extractDatapointFromReasonData() = %v", got)
	}
}
