...
func TestWebhookAuthMiddlewarePassesThroughOtherPaths(t *testing.T) {
	nextCalled := false
	mw := WebhookAuthMiddleware("0123456789abcdef", true, discardLogger(), nil)
...
func TestWebhookAuthMiddlewareAcceptsValidSNSWithoutHeader(t *testing.T) {
	nextCalled := false
	mw := WebhookAuthMiddleware("0123456789abcdef", true, discardLogger(), func(*cloudwatchmetrics.SNSEnvelopeResult) error {
		return nil
	})
	handler := mw(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		nextCalled = true
...
func TestWebhookAuthMiddlewareSkipsAuthWhenDisabled(t *testing.T) {
	mw := WebhookAuthMiddleware("", false, discardLogger(), func(*cloudwatchmetrics.SNSEnvelopeResult) error {
		return errors.New("would-be-bad-sig")
	})
	nextCalled := false
	handler := mw(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		nextCalled = true
	...
	// Non-SNS body, no auth header — should pass when secret auth is disabled.
	req := httptest.NewRequest(http.MethodPost, "/api/v1alpha1/alerts/webhook", strings.NewReader(`{"alarmName":"x","ruleName":"r","state":"ALARM"}`))
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	...
	if !nextCalled {
		t.Fatal("expected request to reach next handler when auth disabled")
	}
}
...
func validSNSBody() string {
	return `{
		...
		"Message": "{\"AlarmName\":\"oc-metrics-alert-ns.cGF5bWVudHM.rn.aGlnaC1jcHU.hash\",\"NewStateValue\":\"ALARM\",\"NewStateReason\":\"Threshold Crossed: 1 datapoint [5.0 (23/04/26 10:00:00)] was greater than the threshold (3.0).\",\"StateChangeTime\":\"2026-04-23T10:00:05Z\"}"
	}`
}
